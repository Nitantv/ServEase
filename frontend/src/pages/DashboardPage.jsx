...
const DashboardPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      ...
      try {
        ...
        const addressesRes = await fetch(process.env.REACT_APP_BACKEND_URL+"/dash/user/addresses", { credentials: "include" });
        if (addressesRes.ok) {
        ...
        if (userInfo.role === 'Customer') {
          const bookingsRes = await fetch(process.env.REACT_APP_BACKEND_URL+"/dash/customer/my-bookings", { credentials: "include" });
          if (bookingsRes.ok) {
            setUpcomingBookings(await bookingsRes.json());
          }
        } else { // User is a Worker
          // Fetch the worker's professional profile
          const profileRes = await fetch(process.env.REACT_APP_BACKEND_URL+"/dash/worker/my-profile", { credentials: "include" });
          if (profileRes.ok) {
            setWorkerProfile(await profileRes.json());
          }
          // Fetch the worker's upcoming bookings
          const bookingsRes = await fetch(process.env.REACT_APP_BACKEND_URL+"/dash/worker/my-bookings", { credentials: "include" });
  ...
  useEffect(() => {
    ...
    const fetchWorkers = async () => {
      setIsLoading(true);
      setWorkers([]);
      const apiUrl = process.env.REACT_APP_BACKEND_URL+`/dash/workers?role=${activeTab}&address=${encodeURIComponent(selectedAddress)}`;
      try {
  ...
  const handleCancelBooking = async (bookingId) => {
    ...
    try {
      const response = await fetch(process.env.REACT_APP_BACKEND_URL+`/dash/booking/${bookingId}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      if (!response.ok) {
